...
DEVICE_PII_DICT = {	
	'3A131JEHN15160':{
		'EMIAL': 'jane.doe.oh.no.666@gmail.com',
		'GSF':'30988DAC1A2B45B6'
	},
}
...
class DEVICE:
	...
	def get_bluetooth_mac(self):
		...
		output = adb_object.run_cmd(f'shell settings get secure bluetooth_address')
		ble_mac = ""
	...
	def get_bluetooth_name(self):
		...
		device_id = self.device_id
		adb_object = ADB(Device(device_id))
		output = adb_object.run_cmd(f'shell dumpsys bluetooth_manager | grep name')
		ble_name = ""
		ble_name = output.split('name:', 1)[-1].strip()
	...
	def get_contacts(self):
		...
		contact_list = []
		adb_object = ADB(Device(device_id))
		output = adb_object.run_cmd('shell su content query --uri content://contacts/phones/')
		for line in output.split('\n'):
			line_list = line.split(', ')
			contact_dict = dict()
...
def get_hash_values(value):
	...
	# print(f'DECODED MD5: {encoded.decode("utf-8")}')
...
def detect_pii(traffic_log, hashed_pii_dict):	
	...
	# print(f'detect_pii traffic log: {traffic_log.request_body}')
...
def init_pii_dict(device_serial):
	# DEVICE_PII_DICT[device_serial] = {} 
	# init device PII dict	
	device_object = DEVICE(device_serial)
	wifi_info_dict = device_object.get_wifi_info()
	bluetooth_mac = device_object.get_bluetooth_mac()
 
	for key in wifi_info_dict:
		DEVICE_PII_DICT[device_serial][key] = wifi_info_dict[key]

	DEVICE_PII_DICT[device_serial]['BLE MAC'] = device_object.get_bluetooth_mac()
	DEVICE_PII_DICT[device_serial]['BLE NAME']= device_object.get_bluetooth_name()
	DEVICE_PII_DICT[device_serial]['AID'] = device_object.get_advertising_id()[0]
	# DEVICE_PII_DICT[device_serial]['AID']= 'cb478862-3b6b-4521-a998-5626ac14af51'
...
def main():
	...
	for log in traffic_logs:
		...
		for log_parameter in traffic_parameters:	
			...
			if len(requested_pii_set) == 0:
				continue
			# print(f'Requested PIIs in request: {requested_pii_set}')		
			for key in requested_pii_set:
				template_str = f'"{log_parameter.package_name}","{host_name_url}","{key}"'				
				dump_csv_line_set.add(template_str)	

	# print(dump_csv_line_set)
	# print(len(dump_csv_line_set))

	package_name_list = []
	host_url_list = []
	key_list = []
	for element in dump_csv_line_set:
		parts = element.strip('"').split('","')
		package_name_list.append(parts[0])
		host_url_list.append(parts[1])
		key_list.append(parts[2])
